...
+ (NSString *)passwordForService:(NSString *)service account:(NSString *)account error:(NSError **)error{
    ...
    if (status != errSecSuccess && error != NULL) {
        *error = [[NSError alloc] initWithDomain:@"com.olla,keychain.error" code:status userInfo:@{@"message":[ZZKeychain keychainMessageFromStatus:status]}];
        return nil;
    }
...
+ (BOOL)setPassword:(NSString *)password forService:(NSString *)service account:(NSString *)account error:(NSError **)error{
    ...
    [ZZKeychain deletePasswordForService:service account:account error:nil];
    if (!password) {
        return YES;
    }
    NSDictionary *query = @{(__bridge id)kSecClass:(__bridge id )kSecClassGenericPassword,
